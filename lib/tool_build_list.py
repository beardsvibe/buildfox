...
def build_targets_dict(ir):
	targets = {}
	for i, build in enumerate(ir.builds):
...
def needed_to_execute_indexes(ir, end_targets, targets_dict = None):
...
def needed_to_execute_builds(ir, need_to_execute_indexes):
	builds = []
	for i, build in enumerate(ir.builds):
...
def build_list(ir, end_targets):
	need_to_execute_indexes, all_inputs = needed_to_execute_indexes(ir, end_targets)
	builds = needed_to_execute_builds(ir, need_to_execute_indexes)
	return builds, all_inputs

# return build list for variation
def variation_build_list(ir, variation = None):
	return build_list(ir, end_targets(ir, variation))